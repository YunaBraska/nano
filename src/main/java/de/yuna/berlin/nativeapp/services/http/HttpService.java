        ...
        isReady.set(false, true, state -> {
            ...
            try {
                ...
                server.createContext("/", exchange -> {
                    ...
                    try {
                        context.sendEventReturn(EVENT_HTTP_REQUEST.id(), httpRequest).responseOpt(HttpResponse.class).ifPresentOrElse(
                            response -> sendResponse(exchange, response),
                            () -> context.sendEventReturn(EVENT_HTTP_REQUEST_UNHANDLED.id(), httpRequest).responseOpt(HttpResponse.class).ifPresentOrElse(
                                response -> sendResponse(exchange, response),
                                () -> sendResponse(exchange, new HttpResponse(404, "Page not found".getBytes(), new HashMap<>()))
                            )
                        );
                    } catch (final Exception e) {
                        context.sendEventReturn(EVENT_APP_UNHANDLED.id(), new Unhandled(context, httpRequest, e)).responseOpt(HttpResponse.class).ifPresentOrElse(
                            response -> sendResponse(exchange, response),
